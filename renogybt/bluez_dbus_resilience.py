...
class BlueZAdapterMonitor:
    ...
    async def check_adapter_state(self) -> dict:
        """Check current BlueZ adapter state."""
        bus = None
        try:
            ...
            logger.error("Timeout checking adapter state - D-Bus may be hung")
            return {'healthy': False, 'error': 'timeout'}
        except Exception as e:
            logger.error("Error checking adapter state: %s", e)
    ...
    async def power_cycle_adapter(self, delay: float = 1.0) -> bool:
        """Power cycle adapter to recover from stuck state."""
        import time
        
        current_time = time.time()
        if current_time - self._last_reset_time < 3600:
            if self._reset_count >= self._max_resets_per_hour:
                logger.error("Too many resets (%d) in last hour - refusing", self._reset_count)
                return False
        else:
            self._reset_count = 0
        ...
        try:
            ...
            logger.info("Adapter power cycle complete (reset %d/%d this hour)",
                       self._reset_count, self._max_resets_per_hour)
            return True
            
        except Exception as e:
            logger.error("Error during power cycle: %s", e)
            return False
        finally:
            if bus:
                bus.disconnect()
